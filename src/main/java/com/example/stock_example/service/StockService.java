...
public class StockService {
    ...
    1. Pessimistic Lock
        ...
        - exclusive lock 을 걸게되면 다른 트랜잭션에서는 lock 이 해제되기 전까지 데이털르 가져갈 수 없게 됨
        - 데드락 발생 가능성
    2. Optimistic Lock
        - 실제 Lock 을 이요하지 않고 버전을 이용하여 정합성을 맞춤
        - 데이터를 읽은 후 update 를 수행할 때 내가 읽은 버전이 맞는지 확인하여 업데이트
            - 내가 읽은 버전과 맞지 않는다면 application 에서 다시 읽은 후에 작업 수행
    3. Named Lock
        - metadata lock
        - lock 획득 후 해제하기 전까지 다른 세션에서는 해당 lock 을 획득할 수 없음
        - transaction 이 종료될 때 lock 이 자동으로 해제되지 않음, 별도 명령어로 해제를 수행해주거나 선점시간이 끝나야 ㅎ제됨
     */
